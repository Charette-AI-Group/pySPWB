"""RPC-III and HEAD acoustics import in the Time Processing window.

Both go through one shared flow (``_import_channels``): list the channels,
let the user pick, import. These tests drive that flow with the channel
dialog auto-accepted, and check the two things a user would notice - the
signals land in the store, and a failure surfaces as a message rather than
a traceback.
"""
import os

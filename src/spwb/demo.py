"""The demonstration datasets, one set per analysis window.
...
This lives in the package rather than in ``tools/`` so that the datasets
reach everyone who installs SPWB, not only people working from a git
checkout: the user manuals teach through these files, and a manual whose
first instruction cannot be followed is not much of a manual. Three ways
in, all of them reaching :func:`write_demo_data`:

* **File > Create Demo Data ...** in the application, which asks where to
  put them;
* ``python tools/make_demo_data.py [folder]`` from a checkout;
* ``from spwb.demo import write_demo_data`` in a script or a notebook.

Files are written in SPWB's native HDF5 format, so they open with
File > Open > SPWB / HDF5. Qt-free, like everything outside
:mod:`spwb.gui`; writing needs ``h5py`` (the ``io`` extra), while the
synthesis itself needs only numpy and scipy.
"""
...
__all__ = ["DATASET_COUNT", "write_demo_data"]

FS = 8192.0          # 1 Hz bins with a 8192-point block
FS_AUDIO = 51200.0   # SPWB's default rate, needed to reach 16 kHz

#: Fixed so the noise in these datasets is the same everywhere. The manuals
#: quote measured values from the noisy signals - "gauss rms 1.00225" - and
#: those are only checkable if everyone's copy is identical.
SEED = 20260815
...
def fft_known_amplitudes(out):
    ...
    signals = [
        sig("Three tones 1.00 0.50 0.25", y, FS, "V",
            ...
            Suggested_Settings="df = 1 Hz, hanning, function type "
...
Regenerate them at any time from the application:
  File > Create Demo Data ...
or, from a source checkout:
  python tools/make_demo_data.py [folder]
...
#: how many .h5 files :func:`write_demo_data` writes, README.txt aside
DATASET_COUNT = sum(len(builders) for _window, builders in BUILDERS)
...
def write_demo_data(
    ...
    """Write the whole demonstration set into ``folder``.

    Creates the folder if it does not exist, and **overwrites** any demo
    file already there. The synthesis is seeded, so the *signals* come back
    identical every time - the HDF5 containers around them do not compare
    byte-for-byte, since the format records its own internal bookkeeping.

    ``progress`` is called after each file as ``(done, total, path)``,
    which is enough to drive a status bar or a progress dialog. It is
    called ``DATASET_COUNT`` times; the README is written afterwards.

    Returns the data files written, in the order the manuals number them.
    """
    # Reseed per call, not once at import. The builders draw from a shared
    # generator, so its state carries over - which never showed while this
    # was a script run once per process, but means the second use in a
    # session would otherwise produce different noise from the first. The
    # menu entry can be used repeatedly, so it has to be reset here.
    global RNG
    RNG = np.random.default_rng(SEED)
    ...
    written: list[Path] = []
    for _window, builders in BUILDERS:
        for build in builders:
            written.append(build(out))
            if progress is not None:
                progress(len(written), DATASET_COUNT, written[-1])
    write_readme(out)
    return written
